 ...
 * ==========================================
 * Utilitaires pour parser les fichiers CSV, Excel, JSON, Parquet et PDF.
 ...
 * Note: apache-arrow est optionnel et doit être installé séparément si nécessaire
 * Cette fonction est désactivée par défaut car apache-arrow peut ne pas être installé
 */
async function parseParquetWithArrow(
  _arrayBuffer: ArrayBuffer,
  _maxRows: number
): Promise<{ data: Record<string, unknown>[]; headers: string[] }> {
  // Apache Arrow est désactivé pour éviter les warnings de module non trouvé
  // Pour activer, installez apache-arrow: npm install apache-arrow
  ...
  */
  ...
  throw new Error('Apache Arrow non disponible. Pour activer le support Parquet, installez le package: npm install apache-arrow');
}

/**
 * Parse un fichier PDF (.pdf)
 * Extrait le texte et tente de détecter des tableaux
 * Note: pdf-parse est chargé dynamiquement car c'est un module serveur uniquement
 */
export async function parsePDF(
  ...
  try {
    // Charger pdf-parse dynamiquement (module serveur uniquement)
    const pdfParse = await import('pdf-parse').then(mod => mod.default || mod).catch(() => null);
    ...
    if (!pdfParse) {
      return {
        ...
        rowCount: 0,
        columnCount: 0,
        errors: ['Le support PDF n\'est pas disponible. Veuillez réinstaller les dépendances avec: npm install pdf-parse'],
        warnings,
      };
    }
    ...
    if (!text || text.trim().length === 0) {
      return {
        ...
        rowCount: 0,
        columnCount: 0,
        errors: ['Le fichier PDF ne contient aucun texte extractible. Il pourrait s\'agir d\'un PDF scanné ou d\'images.'],
        warnings,
      };
    }
    ...
    // Tenter de détecter si c'est un tableau
    // Chercher des patterns de données tabulaires
    const tableData: Record<string, unknown>[] = [];
    let headers: string[] = [];
    
    // Pattern pour détecter les lignes de données avec séparateurs
    ...
    let detectedSeparator: RegExp | null = null;
    ...
    if (detectedSeparator) {
      ...
      const dataLines = lines.slice(0, maxRows);
      
      if (dataLines.length > 0) {
        // Première ligne comme en-têtes potentiels
        const firstLineParts = dataLines[0].split(detectedSeparator).map(p => p.trim()).filter(p => p);
        
        // Vérifier si la première ligne ressemble à des en-têtes
        const looksLikeHeader = firstLineParts.length >= 2 && 
          firstLineParts.some(p => /^[A-Za-z\u00C0-\u017F_]/.test(p));
        
        if (looksLikeHeader) {
          headers = firstLineParts.map(cleanColumnName);
          
          // Traiter les lignes suivantes comme données
          for (let i = 1; i < dataLines.length; i++) {
            const parts = dataLines[i].split(detectedSeparator).map(p => p.trim()).filter(p => p);
            if (parts.length > 0) {
              const row: Record<string, unknown> = {};
              headers.forEach((header, idx) => {
                row[header] = parts[idx] !== undefined ? parseValue(parts[idx]) : null;
              });
              tableData.push(row);
            }
          }
        } else {
          // Pas d'en-têtes clairs, créer des colonnes génériques
          const maxCols = Math.max(...dataLines.map(line => line.split(detectedSeparator).filter(p => p.trim()).length));
          headers = Array.from({ length: maxCols }, (_, i) => `colonne_${i + 1}`);
          
          for (const line of dataLines) {
            const parts = line.split(detectedSeparator).map(p => p.trim()).filter(p => p);
            if (parts.length > 0) {
              const row: Record<string, unknown> = {};
              headers.forEach((header, idx) => {
                row[header] = parts[idx] !== undefined ? parseValue(parts[idx]) : null;
      ...
      warnings.push('Données tabulaires détectées dans le PDF. L\'extraction peut ne pas être parfaite.');
    } else {
      // Pas de structure tabulaire détectée, extraire comme paragraphes
      // Créer une structure simple: paragraphe_num, contenu
      headers = ['numero_paragraphe', 'contenu', 'page'];
      ...
      for (const line of lines.slice(0, maxRows)) {
        // Détecter les sauts de page potentiels
        ...
        // Nouveau paragraphe si ligne vide ou point final
        if (line.endsWith('.') && currentParagraph.trim().split('.').length > 3) {
          tableData.push({
            numero_paragraphe: paragraphNum++,
            contenu: currentParagraph.trim(),
            page: currentPage,
          });
          currentParagraph = '';
        }
      ...
      warnings.push('Le PDF a été extrait sous forme de paragraphes. Pour de meilleurs résultats, utilisez un fichier CSV ou Excel.');
    }
    
    // Ajouter les métadonnées du PDF
    warnings.push(`PDF: ${data.numpages} page(s), ${data.numrender} rendu(s)`);
    if (data.info && data.info.Title) {
      warnings.push(`Titre: ${data.info.Title}`);
    }
    ...
    return {
      ...
      columnCount: 0,
      errors: [
        `Erreur de parsing PDF: ${error instanceof Error ? error.message : 'Erreur inconnue'}`,
      ],
      warnings,
    };
  }
...
function parseValue(value: string): string | number | boolean | null {
  ...
  // Boolean
  if (['true', 'vrai', 'oui', 'yes'].includes(trimmed.toLowerCase())) return true;
  if (['false', 'faux', 'non', 'no'].includes(trimmed.toLowerCase())) return false;
  
  // Nombre
  const numValue = trimmed.replace(/[^\d.,\-]/g, '').replace(',', '.');
  ...
  switch (extension) {
    ...
    default:
      return {
        ...
        rowCount: 0,
        columnCount: 0,
        errors: [`Format de fichier non supporté: .${extension}. Formats acceptés: CSV, XLSX, XLS, JSON, Parquet, PDF.`],
        warnings: [],
      };
  }
}
...
export async function validateFile(file: File): Promise<FileValidationResult> {
  ...
  const extension = file.name.split('.').pop()?.toLowerCase();
  if (!['csv', 'xlsx', 'xls', 'json', 'parquet', 'pdf'].includes(extension || '')) {
    errors.push(
      `Format de fichier non supporté (.${extension}). Formats acceptés: CSV, XLSX, XLS, JSON, Parquet, PDF.`
    );
  }
