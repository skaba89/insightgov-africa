  ...
  const refreshSubscription = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // L'API retourne { success: true, subscription: { tier: '...' } }
        const subscriptionTier = data.subscription?.tier || data.tier || 'free';
        setTier(subscriptionTier);
      } else {
        // Ne pas logger les erreurs 404/401 - c'est normal pour les nouveaux utilisateurs
        setTier('free');
      }
    } catch (error) {
      // Erreur réseau ou timeout - utiliser le plan gratuit
      // Ce n'est pas une erreur critique, on continue avec le plan free
      setTier('free');
    } finally {
      setIsLoading(false);
    }
