...
export function DashboardContainer({
  ...
  const displayData = useMemo(() => {
    ...
    // Si on a des données filtrées, les utiliser en priorité
    if (filteredData && filteredData.length > 0) {
      console.log('DashboardContainer.displayData - Using filteredData, length:', filteredData.length);
      return filteredData;
    }
    
    // Sinon, si on a des données du store, les utiliser
    ...
    return demoData;
  }, [filteredData, dataPreview, config]);
  ...
  const handleFilterChange = useCallback((data: Record<string, unknown>[]) => {
    // Si les données filtrées sont identiques aux données originales, c'est qu'il n'y a pas de filtre actif
    if (dataPreview && data.length === dataPreview.length) {
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {(filteredData || comparisonData) && (
          <div className="mb-4 space-y-2">
            {filteredData && filteredData.length !== (dataPreview?.length || 0) && (
              <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                <p className="text-sm text-blue-700 dark:text-blue-300">
                  Affichage de {filteredData.length} enregistrements sur {dataPreview?.length || 0}
                </p>
              </div>
            )}
