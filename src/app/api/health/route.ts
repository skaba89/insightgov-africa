...
function getMemoryUsage(): { total: number; used: number; free: number; usagePercent: number } {
  ...
  const arrayBuffers = (memUsage as unknown as Record<string, number>).arrayBuffers || 0;
  const totalMemory = memUsage.heapTotal + memUsage.external + arrayBuffers;
...
export async function GET(request: Request) {
  ...
  if (!detailed) {
    (healthReport as unknown as Record<string, unknown>).summary = {
